    ...
    async function initialize(isRetry = false) {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                // v2→v3: reading_stats 추가 (bookmarks는 viewer DB에 추가)
                if (oldVersion < 3) {
...
// ============================================
// NetworkManager - 네트워크 상태 + 자동 재시도
// ============================================
const NetworkManager = (() => {
    let isOffline = false;
    let retryOnlineCallbacks = [];
    ...
    function showBanner(message, type) {
        ...
        if (type === 'online-restored') {
            setTimeout(() => { banner.className = 'network-banner'; }, 3000);
        }
    }
    ...
    function onOnline() {
        ...
        log.info('온라인 복구');
        retryOnlineCallbacks.forEach(cb => { try { cb(); } catch(e) {} });
        retryOnlineCallbacks = [];
    }

    function registerRetryCallback(cb) {
        if (!isOffline) { cb(); return; }
        retryOnlineCallbacks.push(cb);
    }
    ...
    return { initialize, isOffline: () => isOffline, registerRetryCallback };
})();

// ============================================
// FavoritesManager - 즐겨찾기
// ============================================
const FavoritesManager = (() => {
    const STORAGE_KEY = 'fastbook_favorites';
    ...
    function toggle(bookId) {
        ...
        const newFavs = idx === -1 ? [...favs, bookId] : favs.filter(id => id !== bookId);
        localStorage.setItem(STORAGE_KEY, JSON.stringify(newFavs));
        return idx === -1; // true = 추가됨
    }

    return { getFavorites, isFavorite, toggle };
})();

// ============================================
// ReadingStatsManager - 독서 통계
// ============================================
const ReadingStatsManager = (() => {
    function getDB() {
        return Storage.getDB();
    }

    async function saveStats(bookId, sessionDurationMs) {
        try {
            const db = getDB();
            ...
            const updated = {
                ...existing,
                totalMs: (existing.totalMs || 0) + sessionDurationMs,
                sessions: (existing.sessions || 0) + 1,
                lastRead: new Date().toISOString()
            };

            store.put(updated);
        } catch (e) { log.error('통계 저장 실패:', e); }
    }
    ...
    async function getAllStats() {
        try {
            const db = getDB();
            if (!db || !db.objectStoreNames.contains('reading_stats')) return [];
            const tx = db.transaction(['reading_stats'], 'readonly');
            const store = tx.objectStore('reading_stats');
            return new Promise(resolve => {
    ...
    async function openStatsModal(allBooks) {
        const modal = document.getElementById('statsModal');
        if (!modal) return;

        const stats = await getAllStats();
        const statsMap = Object.fromEntries(stats.map(s => [s.bookId, s]));

        const totalBooks = allBooks.length;
        let totalMs = 0;
        stats.forEach(s => totalMs += (s.totalMs || 0));

        // 완독 및 진행 중 카운트 (lastRead 데이터 기반 추정)
        const readingBooks = allBooks.filter(b => b.lastRead).length;
        const completedBooks = stats.filter(s => s.completed).length;

        document.getElementById('statsTotalBooks').textContent = totalBooks;
        document.getElementById('statsCompletedBooks').textContent = completedBooks;
        document.getElementById('statsReadingBooks').textContent = readingBooks;
        document.getElementById('statsTotalTime').textContent = formatTime(totalMs);

        const list = document.getElementById('statsBooksList');
        list.innerHTML = '';
        ...
        if (sortedStats.length === 0) {
            list.innerHTML = '<p style="color:var(--text-tertiary);text-align:center;padding:1rem;font-size:0.875rem;">아직 독서 기록이 없습니다.</p>';
        } else {
            sortedStats.forEach(s => {
                const book = allBooks.find(b => b.id === s.bookId);
                const title = book ? book.title : s.bookId;
                const item = document.createElement('div');
                item.className = 'stats-book-item';
                item.innerHTML = `
                    <span class="book-name" title="${title}">${title}</span>
                    <span class="read-time">${formatTime(s.totalMs)}</span>
                `;
        ...
        document.getElementById('statsModalClose').focus();
    }

    return { saveStats, getAllStats, openStatsModal, formatTime };
})();

// ============================================
// RecentBooksSection - 최근 읽은 책 빠른 접근
// ============================================
const RecentBooksSection = (() => {
    function render(books) {
        ...
        // 최근 읽은 책 (lastRead 있는 것, 최신 순, 최대 10개)
        const recentBooks = books
            .filter(b => b.lastRead)
            .sort((a, b) => {
                const aT = a.lastRead?.lastRead || '0';
                const bT = b.lastRead?.lastRead || '0';
        ...
        recentBooks.forEach(book => {
            ...
            card.setAttribute('aria-label', `${book.title} 계속 읽기`);

            card.innerHTML = `
                <img class="recent-book-cover" src="${book.coverImage}" alt="${book.title}" loading="lazy">
                <div class="recent-book-info">
                    <div class="recent-book-title" title="${book.title}">${book.title}</div>
                    <div class="recent-book-progress">${book.pageCount}페이지</div>
                    <div class="recent-book-progress-bar">
                        <div class="recent-book-progress-fill" style="width:0%"></div>
                    </div>
                </div>
            `;
            ...
            card.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') openBook(); });
...
const APIQueue = (() => {
    ...
    const MIN_INTERVAL_MS = 100; // 요청 사이 최소 간격
...
// Service Worker 등록
// ============================================
if (CONFIG.ENABLE_SERVICE_WORKER && 'serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js')
            .then(reg => log.info('Service Worker 등록 완료:', reg.scope))
            .catch(err => log.warn('Service Worker 등록 실패:', err));
    });
}

// ============================================
// 새 기능 초기화 (initializeApp 이후 훅)
// ============================================
...
const _origSortBooks = sortBooks;
...
const _origLoadBooks_forRecent = loadBooks;
// 대신 displayBooks 내에서 최근 섹션을 항상 갱신
const _displayBooksWithRecent = displayBooks;
displayBooks = function(books) {
    _displayBooksWithRecent(books);
    // allBooks 기준으로 최근 섹션 렌더링
    if (allBooks.length > 0) RecentBooksSection.render(allBooks);
};

// DOM 로드 후 신기능 초기화 (initializeApp보다 늦게)
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(initializeNewFeatures, 0);
});
